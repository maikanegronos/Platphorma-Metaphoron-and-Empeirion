...
export default function Driver() {
  ...
  const updateStatus = useUpdateDriverJobStatus();
  ...
  const onStatus = (job: DriverJob, status: 'in-progress' | 'completed') => {
...
function JobCard({ job, onClaim, onStatus, pending, statusPending, claimed }: { job: DriverJob; onClaim: () => void; onStatus: (job: DriverJob, status: 'in-progress' | 'completed') => void; pending: boolean; statusPending: boolean; claimed: boolean }) {
  ...
  const nextStatus = job.status === 'claimed' ? 'in-progress' : job.status === 'in-progress' ? 'completed' : null;
  return <article className={`rounded-2xl border bg-[#f6f7f6] p-5 shadow-sm ${job.isCustom ? 'border-accent/50' : 'border-[#c5d4d8]'}`} data-testid={`card-job-${job.id}`}><div className="flex items-start justify-between gap-3"><div className="flex gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-[#d6e6ea] text-primary"><Route size={19} /></div><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-bold text-primary">{job.title}</h3>{job.isCustom && <span className="rounded-full bg-accent/15 px-2 py-1 font-mono-ui text-[9px] uppercase tracking-wider text-[#9d543c]">Custom</span>}</div><p className="mt-1 text-xs text-muted-foreground">{formatDate(job.date)} · {job.time}</p></div></div><StatusPill status={job.status} /></div><div className="my-5 grid gap-3 border-y border-[#d6e0e2] py-4 text-sm sm:grid-cols-2"><div className="flex gap-3"><MapPin size={16} className="mt-0.5 shrink-0 text-accent" /><div><p className="text-[10px] uppercase tracking-wider text-muted-foreground">Παραλαβή</p><p className="mt-1 font-semibold text-primary">{job.pickup}</p></div></div><div className="flex gap-3"><Navigation size={16} className="mt-0.5 shrink-0 text-accent" /><div><p className="text-[10px] uppercase tracking-wider text-muted-foreground">Προορισμός</p><p className="mt-1 font-semibold text-primary">{job.destination}</p></div></div></div>{job.stops.length > 1 && <div className="mb-5 -mt-2 flex items-start gap-3 text-sm"><Route size={16} className="mt-0.5 shrink-0 text-accent" /><div><p className="text-[10px] uppercase tracking-wider text-muted-foreground">Στάσεις καθ' οδόν</p><p className="mt-1 font-semibold text-primary">{job.stops.join(' → ')}</p></div></div>}{job.notes && <div className="mb-5 -mt-2 flex items-start gap-3 rounded-xl bg-[#fbf3e6] p-3 text-sm"><FileText size={16} className="mt-0.5 shrink-0 text-accent" /><div><p className="text-[10px] uppercase tracking-wider text-muted-foreground">Σημείωση πελάτη</p><p className="mt-1 text-primary">{job.notes}</p></div></div>}<div className="flex flex-wrap items-center gap-4"><DataTag icon={Users}>{job.passengers} άτομα</DataTag><DataTag icon={Clock3}>{job.distanceKm || '—'} χλμ</DataTag><div className="ml-auto text-right"><p className="font-mono-ui text-[10px] uppercase tracking-wider text-muted-foreground">Αμοιβή</p><p className="font-display text-2xl text-primary">{formatEuro(job.payout)}</p></div></div>{phone && <div className="mt-4 flex items-center justify-between rounded-xl bg-[#e8edf0] px-3 py-2"><span className="text-xs text-muted-foreground">{job.customerName ?? 'Επικοινωνία πελάτη'}</span><div className="flex gap-2"><a href={`https://wa.me/${phone}`} target="_blank" rel="noreferrer" aria-label="WhatsApp πελάτη" className="rounded-lg bg-[#25d366]/15 p-2 text-[#128c7e]"><MessageCircle size={16} /></a><a href={`viber://chat?number=%2B${phone}`} aria-label="Viber πελάτη" className="rounded-lg bg-[#7360f2]/15 p-2 text-[#665cac]"><Phone size={16} /></a></div></div>}{job.status === 'open' ? <ActionButton data-testid={`button-claim-job-${job.id}`} onClick={onClaim} loading={pending} className="mt-5 w-full">Ανάλαβε τη διαδρομή <ArrowRight size={16} /></ActionButton> : nextStatus ? <ActionButton data-testid={`button-status-job-${job.id}`} onClick={() => onStatus(job, nextStatus)} loading={statusPending} className="mt-5 w-full">{nextStatus === 'in-progress' ? 'Ξεκίνα τη διαδρομή' : 'Ολοκλήρωσε τη διαδρομή'} <CheckCircle2 size={16} /></ActionButton> : <div className="mt-5 flex items-center justify-center gap-2 rounded-xl bg-[#dcebe2] py-3 text-sm font-bold text-[#286043]"><CheckCircle2 size={16} /> Η διαδρομή ολοκληρώθηκε</div>}</article>;
}