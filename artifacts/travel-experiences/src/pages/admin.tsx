  ...
  return <div className="min-h-[calc(100dvh-72px)] bg-[#f0eee8] px-5 py-8 md:px-10 md:py-12"><div className="mx-auto max-w-[1280px]">
    ...
    <section id="section-experiences" className="mt-10"><div className="mb-4 flex items-center justify-between"><div><p className="font-mono-ui text-[10px] uppercase tracking-[.2em] text-accent">Catalog</p><h2 className="mt-1 font-display text-3xl text-primary">Εμπειρίες</h2></div>{!editingExperienceId && <button onClick={startNewExperience} data-testid="button-new-experience" className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-primary-foreground"><Plus size={16} /> Νέα εμπειρία</button>}</div>
      {editingExperienceId && experienceForm && <div className="mb-6 rounded-2xl border border-[#d8d5cc] bg-card p-5"><h3 className="mb-4 font-display text-xl text-primary">{editingExperienceId === 'new' ? 'Νέα εμπειρία' : 'Επεξεργασία εμπειρίας'}</h3><div className="grid gap-4 sm:grid-cols-2">
        <label className="text-sm sm:col-span-2"><span className="field-label-dark">Τίτλος</span><input value={experienceForm.title} onChange={(e) => setExperienceForm({ ...experienceForm, title: e.target.value })} className="field-light" data-testid="input-experience-title" /></label>
        <label className="text-sm"><span className="field-label-dark">Τοποθεσία</span><input value={experienceForm.location} onChange={(e) => setExperienceForm({ ...experienceForm, location: e.target.value })} className="field-light" data-testid="input-experience-location" /></label>
        <label className="text-sm"><span className="field-label-dark">Κατηγορία</span><input value={experienceForm.category} onChange={(e) => setExperienceForm({ ...experienceForm, category: e.target.value })} className="field-light" data-testid="input-experience-category" /></label>
        <label className="text-sm"><span className="field-label-dark">Διάρκεια (ώρες)</span><input type="number" step="0.5" min="0.5" value={experienceForm.durationHours} onChange={(e) => setExperienceForm({ ...experienceForm, durationHours: Number(e.target.value) })} className="field-light" data-testid="input-experience-duration" /></label>
        <label className="text-sm"><span className="field-label-dark">Τιμή από (€)</span><input type="number" step="1" min="0" value={experienceForm.priceFrom} onChange={(e) => setExperienceForm({ ...experienceForm, priceFrom: Number(e.target.value) })} className="field-light" data-testid="input-experience-price" /></label>
        <label className="text-sm sm:col-span-2"><span className="field-label-dark">URL φωτογραφίας</span><input value={experienceForm.imageUrl} onChange={(e) => setExperienceForm({ ...experienceForm, imageUrl: e.target.value })} placeholder="https://…" className="field-light" data-testid="input-experience-image" /></label>
        <label className="text-sm sm:col-span-2"><span className="field-label-dark">Περιγραφή</span><textarea rows={3} value={experienceForm.description} onChange={(e) => setExperienceForm({ ...experienceForm, description: e.target.value })} className="field-light w-full resize-none" data-testid="input-experience-description" /></label>
        <label className="text-sm sm:col-span-2"><span className="field-label-dark">Highlights (χωρισμένα με κόμμα)</span><input value={highlightsText} onChange={(e) => setHighlightsText(e.target.value)} placeholder="π.χ. Ξεναγός, Γεύσεις, Sunset stop" className="field-light" data-testid="input-experience-highlights" /></label>
    ...
    <section id="section-pricing" className="mt-10"><div className="mb-4 flex items-center gap-2 text-primary"><Settings2 size={18} /><div><p className="font-mono-ui text-[10px] uppercase tracking-[.2em] text-accent">Pricing engine</p><h2 className="mt-1 font-display text-3xl text-primary">Τιμολόγηση</h2></div></div><p className="mb-4 text-xs text-muted-foreground">Αυτές οι τιμές καθορίζουν αυτόματα κάθε νέα προσφορά τιμής (quote) στην πλατφόρμα.</p>{pricing.isLoading || !pricingForm ? <LoadingState label="Φορτώνουμε τις ρυθμίσεις τιμολόγησης…" /> : <div className="rounded-2xl border border-[#d8d5cc] bg-card p-5"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <label className="text-sm"><span className="field-label-dark">Βασική χρέωση (€)</span><input type="number" step="0.5" min="0" {...pricingField('baseRate')} className="field-light" data-testid="input-pricing-base-rate" /></label>
      <label className="text-sm"><span className="field-label-dark">Χρέωση ανά χλμ (€)</span><input type="number" step="0.05" min="0" {...pricingField('kmRate')} className="field-light" data-testid="input-pricing-km-rate" /></label>
      <label className="text-sm"><span className="field-label-dark">Χρέωση ανά ώρα (€)</span><input type="number" step="0.5" min="0" {...pricingField('hourlyRate')} className="field-light" data-testid="input-pricing-hourly-rate" /></label>
      <label className="text-sm"><span className="field-label-dark">Πολλαπλασιαστής Sedan</span><input type="number" step="0.05" min="0" {...pricingField('vehicleMultiplierSedan')} className="field-light" data-testid="input-pricing-multiplier-sedan" /></label>
      <label className="text-sm"><span className="field-label-dark">Πολλαπλασιαστής Van</span><input type="number" step="0.05" min="0" {...pricingField('vehicleMultiplierVan')} className="field-light" data-testid="input-pricing-multiplier-van" /></label>
      <label className="text-sm"><span className="field-label-dark">Πολλαπλασιαστής Minibus</span><input type="number" step="0.05" min="0" {...pricingField('vehicleMultiplierMinibus')} className="field-light" data-testid="input-pricing-multiplier-minibus" /></label>
      <label className="text-sm"><span className="field-label-dark">Πολλαπλασιαστής Bus</span><input type="number" step="0.05" min="0" {...pricingField('vehicleMultiplierBus')} className="field-light" data-testid="input-pricing-multiplier-bus" /></label>
      <label className="text-sm"><span className="field-label-dark">Δωρεάν επιβάτες (έως)</span><input type="number" step="1" min="0" {...pricingField('freePassengers')} className="field-light" data-testid="input-pricing-free-passengers" /></label>
      <label className="text-sm"><span className="field-label-dark">Χρέωση ανά επιπλέον επιβάτη (€)</span><input type="number" step="0.5" min="0" {...pricingField('extraPassengerRate')} className="field-light" data-testid="input-pricing-extra-passenger-rate" /></label>
      <label className="text-sm"><span className="field-label-dark">Προμήθεια πλατφόρμας (0–1, π.χ. 0.18 = 18%)</span><input type="number" step="0.01" min="0" max="1" {...pricingField('platformFeePercent')} className="field-light" data-testid="input-pricing-platform-fee" /></label>
